      ...
      <div className="blog">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSortBy("newest")} className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-medium transition-all ${sortBy === "newest" ? "bg-pink-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}><ArrowDown size={14} /> Newest</button>
            <button onClick={() => setSortBy("oldest")} className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-medium transition-all ${sortBy === "oldest" ? "bg-pink-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}><ArrowUp size={14} /> Oldest</button>
            <button onClick={() => setSortBy("alphabetical")} className={`flex items-center gap-1 px-3 py-1 rounded text-xs font-medium transition-all ${sortBy === "alphabetical" ? "bg-pink-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>A-Z</button>
          </div>
        </div>
        ...
        {allTags.length > 0 && (
          <div className="mb-6 pb-4 border-b border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              {allTags.map(tag => (
                <button key={tag} onClick={() => setSelectedTags(prev => prev.includes(tag) ? prev.filter(t => t !== tag) : [...prev, tag])} className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${selectedTags.includes(tag) ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>{tag}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-4 pt-6 border-t border-gray-200">
            <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="px-3 py-1 rounded bg-gray-200 text-sm font-medium disabled:opacity-50">Previous</button>
            <span className="text-sm text-gray-600">Page {currentPage} of {totalPages}</span>
            <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="px-3 py-1 rounded bg-gray-200 text-sm font-medium disabled:opacity-50">Next</button>
          </div>
        )}
      </div>
    </>
